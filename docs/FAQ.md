...
5. карточки зависли в `analysis_status=other` после прошлых сбоев — Auto-AI берёт
   только `none`. При включении «Авто» и при старте core они возвращаются в очередь;
   либо `./scripts/local-ai-bootstrap.sh` (сброс other/analyzing).

Полный пересбор на Mac:

```bash
cd /Users/rinat/Documents/work/platform/zakupki-platform
./scripts/local-ai-bootstrap.sh
# UI → включить «Авто»
# docker compose logs -f core analizator
```
